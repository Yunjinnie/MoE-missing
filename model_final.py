...
def product_of_experts(mu_list, logvar_list, calibration=1.0):
    ...
    # precision_list = []
    # for lv in logvar_list:
    #     # lv: (B, E)
    #     # precision: (B, E)
    #     var = torch.exp(lv)
    #     var_mean = var.mean(dim=1, keepdim=True) # (B, 1)
    #     prc_mean = 1.0 / (var_mean + 1e-8) # (B, 1)
    #     precision_list.append(prc_mean)
    precision_sum = sum(precision_list)
    # print(precision_sum.shape) # 왜 list? shape: (B, E) = torch.Size([8, 256])
    # mu_k는 (B, d), prec_mean_list[k]는 (B, 1) broadcast
    mu_fused = sum(mu * prec for mu, prec in zip(mu_list, precision_list)) / (precision_sum)
    var_fused = 1.0 / (precision_sum) # (B, 1)
    #print(var_fused)
...
def jsd_loss(a, b, temperature=0.1): # 0.1
...
class LearnableTokenAttention(nn.Module):
    ...
    def forward(self, features, apply_mask=None):
        ...
        return attn_out.mean(dim=0) # (B, d)
...
class GatingNetwork(nn.Module):
    ...
    def forward(self, x, variance=None, alpha=1.0, beta=1.0, raw_logits=False):
        # x: (B, d), variance: (B, E)
        logits = self.fc(x) # (B, E)
        if variance is not None:
            precision = 1.0 / (variance + 1e-8) # (B, E)
            logits = alpha * logits + beta * torch.log(precision.mean(dim=1, keepdim=True) + 1e-8)
        if raw_logits:
            return logits
    ...
    def forward(self, reps_sh, reps_sp, present):
        ...
        for key, expert in self.experts_sh_pair.items():
            ...
            if i in present and j in present:
                ...
                #expert_names.append(1)
                #mu, lv = expert(inp)
            else:
                # fallback via attention: token -> feature to full-shared
                token = self.tokens_pair[key] #(d,) 
                query = token.unsqueeze(0).unsqueeze(1).expand(B,1,-1) # dim
                #query = token.unsqueeze(0).expand(B,dim) # -1
            ...
            mus.append(mu)
            logvs.append(lv)
            expert_names.append(i in present and j in present)
        ...
        for m, expert in self.experts_sp.items():
            ...
            if m in present:
                ...
                mu = torch.zeros_like(mu_all)
                lv = torch.full_like(mu, fill_value=+10.0) 
                expert_names.append(False)
            mus.append(mu)
            logvs.append(lv)
        ...
        #gate_logits = self.gate(mu_all, var_mean, raw_logits=True) # (B, E)
        gate_logits = self.gate(avg_sh, var_mean, raw_logits=True) # (B, E)
        mask = torch.tensor(expert_names, device=device).unsqueeze(0).expand(B,-1)
        logits = gate_logits.masked_fill(~mask, float('-inf'))
        ...
        precisions = [1.0/var for var in variance] #1.0 / (var_mean + 1e-8)           # (B, E)
        
        # 6) 마스크 만들어서 present 되지 않은 expert + 학습된 적 없는 expert의 logit을 -inf로
        # expert_avail: 길이 7 tensor, 1.0 = available, 0.0 = mask out
        #expert_avail = torch.zeros(7, device=device)
        # full-shared 은 항상
        #expert_avail[0] = 1.0
        # pairwise
        # for idx,key in enumerate(self.experts_sh_pair.keys(), start=1):
        #     i,j = key[0],key[1]
        #     expert_avail[idx] = 1.0 if (i in present and j in present) else 0.0
        # # specific
        # for idx,m in enumerate(['audio','vision','text'], start=1+len(self.experts_sh_pair)):
        #     expert_avail[idx] = 1.0 if (m in present) else 0.0

        # broadcast to (B,7)
        #mask = expert_avail.unsqueeze(0).expand(B, -1)  
        # masking
        #gate_logits = gate_logits.masked_fill(mask==0, float('-inf'))
        # 7) softmax: 실제 weight (B,7)
        ...
        fused_mu, fused_lv = product_of_experts(
            [w.unsqueeze(-1) * m for (w,m) in zip(gate_w.T, mus)],
            logvs
        )
...
class MMDisentangled(nn.Module):
    def __init__(self, input_dim: dict, d_model=256,
        ...
        self.classifier = nn.Linear(latent_shared, num_classes) # * num_modalities
