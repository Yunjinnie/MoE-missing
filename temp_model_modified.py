...
class DisentangledModalEncoder(nn.Module):
    def __init__(self, feature_dim, latent_dim_shared, latent_dim_specific):
        super(DisentangledModalEncoder, self).__init__()
        """
        feature_dim: 입력 feature 차원 (예: pretrained ViT/BERT/Librosa feature): {'vision': 20, 'audio': 5, 'text': 768}
...
class MMDisentangled(nn.Module):
    def __init__(self, modality_feature_dims, latent_dim_shared, latent_dim_specific, num_classes):
        ...
        self.num_modalities = len(modality_feature_dims)
        ...
        self.encoders = nn.ModuleList([
            DisentangledModalEncoder(dim, latent_dim_shared, latent_dim_specific) for dim in modality_feature_dims
        ])
        ...
        self.shared_fusion_linear = nn.Linear(self.num_modalities * latent_dim_shared, latent_dim_shared)
        # Transformer Encoder with MoE를 통한 최종 예측 (shared 정보 사용)
        self.transformer_moe = TransformerEncoderMoE(embed_dim=latent_dim_shared, num_tokens=self.num_modalities, num_classes=num_classes)
    ...
    def forward(self, inputs):
        ...
        for i, x in enumerate(inputs):
            z_shared, mu_shared, logvar_shared, z_specific, mu_specific, logvar_specific = self.encoders[i](x)
        ...
        modality_mask = torch.ones((inputs[0].shape[0], self.num_modalities), device=fused_shared.device)
