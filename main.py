...
def train_and_evaluate(args, seed):
    ...
    for epoch in trange(args.train_epochs):
        ...
        if epoch < args.warm_up_epochs:
            ...
            #print(np.array(all_probs).shape) # (229, 2)
            #print(np.array(all_labels).shape)  # (229,) v 또는 (229, 1)인지 확인
            #print(outputs.shape)  # torch.Size([5, 2]), (batch_size, num_classes)인지 확인


            val_acc = accuracy_score(all_labels, all_preds)
            val_f1 = f1_score(all_labels, all_preds, average='macro') # unweighted
            val_auc = roc_auc_score(all_labels, all_probs) if args.data == 'mimic' or 'mosi' else roc_auc_score(all_labels, all_probs, multi_class='ovr')
            # val_auc = roc_auc_score(all_labels, np.array(all_probs)[:, 1]) if args.data == 'mimic' else roc_auc_score(all_labels, np.array(all_probs), multi_class='ovr')
    ...
    test_auc = roc_auc_score(all_labels, all_probs) if args.data == 'mimic' or 'mosi' else roc_auc_score(all_labels, all_probs, multi_class='ovr')
