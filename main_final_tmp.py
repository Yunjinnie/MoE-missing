...
def parse_args():
    ...
    parser.add_argument('--w_disent', type=float, default=1e-3, help='weight for disentanglement losses (JSD, orth, MI)') # 1.0
    parser.add_argument('--w_gate', type=float, default=1e-1, help='weight for gate regularization loss') # 1e-2
...
def compute_disentangle_mi_loss(shared_reps, specific_reps, mus_shared, mus_specific, y_onehot, critics):
    """
    ...
    return L_jsd, L_orth, L_info_s, L_club_s_cond, L_shared, L_unique
...
def train_one_epoch(model, dataloader, optimizer, criterion, critics, device, args):        
    ...
    for batch_samples, batch_labels, batch_mcs, batch_observed in dataloader:
        ...
        # raw losses
        L_jsd, L_orth, L_info_s, L_club_s_cond, L_shared, L_unique = compute_disentangle_mi_loss(
            shared_reps, specific_reps, mus_s, mus_u, y_onehot, critics)
...
def train_and_evaluate(args, seed, save_path=None):
    ...
    for epoch in range(1, args.train_epochs+1):
        ...
        if epoch < args.warm_up_epochs:
            print(f"[Seed {seed}/{args.n_runs-1}] [Warm-Up Epoch {epoch+1}/{args.warm_up_epochs}] Task Loss: {np.mean(train_loss):.2f}")
        else:
            # 2) validate
